...
if st.button("🚀 Validar Documentos", use_container_width=True):
    if not pedido_file or not certidao_file:
        ...
        with st.spinner("Analisando arquivos..."):
            try:
                # Limpa a chave do CRO da sessão anterior
                st.session_state.pop("chave_cro_detectada", None)

                pedido_bytes = pedido_file.read()
                certidao_bytes = certidao_file.read()

                # Extrai o texto e valida se é Nato-Digital
                texto_pedido = extrair_texto(pedido_bytes)
                texto_certidao = extrair_texto(certidao_bytes)

            except ValueError as erro_pdf:
                st.warning(f"⚠️ {erro_pdf}")
                st.stop()
            except Exception as erro_geral:
                st.error(f"Erro ao ler arquivos PDF: {erro_geral}")
                st.stop()

            # --- CORREÇÃO DE INDENTAÇÃO: O FLUXO CONTINUA AQUI ---
            ...
            # Bloqueio Imediato se arquivos estiverem trocados
            ...
            if conselho_detectado == "CRF":
                ...
                if codigo_autenticacao:
                    ...
                    status_conselho = {"autentica": False, "mensagem": "Código de autenticação não encontrado no documento."}
                ...
                status_conselho = {
                    "autentica": True, 
                    "mensagem": f"Chave de autenticação: {chave_autenticacao} \n\nO código foi extraído com sucesso! Utilize o link abaixo para acessar a validação externa."
                }
            ...
            try:
                data_validade = datetime.strptime(data_validade_str.strip(), "%d/%m/%Y").date()
            ...
            if status_compativel_final:
                ...
                lista_motivos = []
                if certidao_vencida:
                    lista_motivos.append("Certidão VENCIDA")
                
                erros_comparacao = resultado_comparacao.get("erros", [])
                if erros_comparacao:
                    lista_motivos.extend(erros_comparacao)
                
                motivos_str = " e ".join(lista_motivos) if lista_motivos else "Divergência de dados não especificada"
